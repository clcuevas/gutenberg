...
function block_core_table_of_contents_get_headings( $post ) {
	...
	// Suppress errors that occur during parsing. HTML5 elements aren't
	// supported (as of PHP 7.4) and There's no way to guarantee that the markup
	// is valid anyway.
	// The HTML5 issue lies with libxml2:
	...
	// Clear errors to avoid a memory leak.
	libxml_clear_errors();
	...
	// iterate over that.
	$iframes = iterator_to_array(
		$document_el->getElementsByTagName( 'iframe' )
	);

	foreach ( $iframes as $iframe ) {
		block_core_table_of_contents_delete_node_and_children( $iframe );
	}

	// Get and remove template elements for the same reason as iframes.
